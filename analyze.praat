...
form Save acoustics to file
	...
	sentence Directory ./data/all_textgrids/

        comment Where do you want to save the results?

	text textfile duration_data.txt
endform
...
for ifile to numberOfFiles
	...
	if right$ (filename$, 9) = ".TextGrid"
		...
		soundid$ = left$(filename$, (length (filename$) - 9))
		
		subj$ = mid$(soundid$, 2, 3)
		item$ = mid$(soundid$, 7, 3)
...
#		Open long sound file... 'directory$''filename$'
		...
		for interval from 1 to number_of_intervals
			...
			if text$ <> ""
				...
				utterance_start = Get starting point... 2 interval
				utterance_end = Get end point... 2 interval
...
#				resultline$ = "'" + "'soundid$'" + "'" + tab$ + "'" + "'text$'" + "'" + tab$ + "'duration'" + tab$ + "'min'" +tab$+ "'max'" +tab$+ "'mean'" +tab$+ "'slope'" + newline$
				...
				resultline$ = "'subj$'" + tab$ + "'item$'" + tab$ + "'text$'" + tab$ + "'utterance_duration'" + newline$
