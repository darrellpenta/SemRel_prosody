...
cat(rep(c("-"), times = 40, quote = F), "\n")
pros.d1.i <- lmer(seconds ~  integ  + plaus + n1.len + p1.len + a1.len + n2.len + fr.pos.wd + (1+integ|subject) + (1+integ|item), data = d.d1, REML = TRUE)
...
cat(rep(c("-"), times = 40, quote = F), "\n")
pros.d1.r <- lmer(seconds ~  relat + plaus + n1.len + p1.len + a1.len + n2.len + fr.pos.wd + (1 + relat|subject) + (1+relat|item), data = d.d1, REML = TRUE)
...
coeff.table.1 <- data.frame (
  ...
  Beta   = c(
    ...
    co.d1.a<- cbind(as.data.frame(t(t(colnames(pros.d1.a@pp$X)))),as.data.frame(t(t(pros.d1.a@beta))))[2,2],
    ...
    co.n1.a<- cbind(as.data.frame(t(t(colnames(pros.n1.a@pp$X)))),as.data.frame(t(t(pros.n1.a@beta))))[2,2],
    ...
    co.p1.a<- cbind(as.data.frame(t(t(colnames(pros.p1.a@pp$X)))),as.data.frame(t(t(pros.p1.a@beta))))[2,2],
    ...
    co.d2.a<- cbind(as.data.frame(t(t(colnames(pros.d2.a@pp$X)))),as.data.frame(t(t(pros.d2.a@beta))))[2,2],
    ...
    co.a1.a<- cbind(as.data.frame(t(t(colnames(pros.a1.a@pp$X)))),as.data.frame(t(t(pros.a1.a@beta))))[2,2],
    ...
    co.n2.a<- cbind(as.data.frame(t(t(colnames(pros.n2.a@pp$X)))),as.data.frame(t(t(pros.n2.a@beta))))[2,2]))

coeff.table.1.ir <- subset(coeff.table.1, Factor != "Association")
...
g1     <- ggplot(data = coeff.table.1.ir, aes(x = POS, y = Beta, fill = Factor)) +
...
ggsave("figures/Coefficients X Part of Speech (No Residuals) (Fixed Effects: Int. & Rel.).png")
